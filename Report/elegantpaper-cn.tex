...
\keywords{邮件分类，并行计算，机器学习，MapReduce，Spark}
\end{abstract}
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{2.5cm}|p{10cm}|}
    ...
    小组成员  &   负责任务  \\
    \hline
    曹佳涵 & 文本预处理，文本特征选择算法实现和改进，文本特征向量权重计算TF-IDF算法的实现，Spark Random Forest模型以及实验结果分析，实验报告框架搭建、内容撰写、结果总结及排版修改。\\
    \hline
...
\begin{figure}[H]
  ...
  \centering
  \includegraphics[height=3.5in]{BayesTrainResult.png}
  \caption{训练Job输出结果 \label{fig:BayesTrainResult}}
  \label{fig:side:a}
  \end{minipage}
  \begin{minipage}[t]{0.33\linewidth}
  \centering
  \includegraphics[height=3.5in]{GetClassNum.png}
  \caption{类编号与对应文件数 \label{fig:GetClassNum}}
  \label{fig:side:b}
  \end{minipage}
  \begin{minipage}[t]{0.33\linewidth}
  \centering
  \includegraphics[height=3.5in]{BayesPredictResult.png}
...
在Reduce阶段，只需要对map对每个测试样本发射的K个最近的样本类别，取其中数量最多的类别作为预测结果。\par
伪代码见Algorithm 11.
\begin{algorithm}[!htb]
  \caption{KNN的预测过程}
  \label{alg:Framwork}
  ...
  \Function {map}{$lineno$, $testLine$}
    ...
    \For{$instance$ in $trainList$}
        ...
        \If{$dis <$ distance List中最大的样本$maxInstance$}
...
\begin{figure}[H]
  ...
  \centering
  \includegraphics[width=3.5in]{BayesRecall.png}
  \caption{两种方式下20个类的Recall \label{fig:BayesRecall}}
  \label{fig:side:a}
  \end{minipage}
  \begin{minipage}[t]{0.5\linewidth}
  \centering
  \includegraphics[width=3.5in]{BayesPrecision.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{BayesF1.png}
...
使用Hadoop MapReduce实现KNN的结果与其他分类方法的比较图如5.5中折线图所示。从结果中可以看到，KNN的表现是所有分类器中最差的。\par
KNN算法常见的缺陷是由于类别不平衡问题导致投票确定预测类别时产生偏差。但根据前文的分析，训练样本数据是非常均衡的，所以恰好避开了这一缺陷造成的影响。\par
KNN算法的精度非常依赖于数据的分布。当同一类别的数据，在样本特征空间上分布不聚拢时，便会在预测上产生偏差。因此可以通过降维技术(主成分分析、因子分析等)，或者选择更合适的距离度量指标，可以提高KNN算法的精度。
...
当迭代次数选择100时，准确度以及各个类别的Precision、Recall和F1-Score如5.5中折线图所示。\par
分析实验结果，可以看到SVM在稀疏短文本分类的任务下表现较好，可以达到87\%以上的准确率。但其并没有达到90\%的准确率，这与SVM的参数选择与迭代次数，以及数据的分布有着密切的关系。本次实验的训练样本容量为19997条，这对于短文本分类任务来说，样本数量相对较少，因此使用SVM容易产生过拟合现象。\par
这里使用的SVM多分类方法是“一对多法”，其优点是训练N个分类器，个数较少，其分类速度相对较快。但横向与Naive Bayes的结果做比较，SVM的效果并不是很好，分析认为，每个分类器的训练都是将全部样本作为训练样本，这样在求解二次规划问题时，训练速度会随着训练样本的数量的增加而急剧减缓；同时由于负类样本的数据要远大于正类样本的数据，从而出现样本部队称的情况，且这种情况随着训练数据的增加而趋向严重。在后续的研究中可以引用惩罚因子等方法来缓解类别不平衡的问题。\par


\subsection{各分类模型结果对比}
我们将各个分类模型在测试数据上的Accuracy以及各个类别上的Precision，Recall，F1-score指标进行汇总和绘图分析，结果如下：\par

\begin{figure}[H]
  \begin{minipage}[t]{0.5\linewidth}
  \centering
  \includegraphics[width=3.5in]{all_precision.jpg}
  \caption{各分类预测模型测试集Precision比较}
  \label{fig:side:a}
  \end{minipage}
  \begin{minipage}[t]{0.5\linewidth}
  \centering
  \includegraphics[width=3.5in]{all_recall.jpg}
  \caption{各分类预测模型测试集Recall比较}
  \label{fig:side:b}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
  \begin{minipage}[t]{0.5\linewidth}
  \centering
  \includegraphics[width=3.5in]{all_f1.jpg}
  \caption{各分类预测模型测试集F1-score比较}
  \label{fig:side:a}
  \end{minipage}
  \begin{minipage}[t]{0.5\linewidth}
  \centering
  \includegraphics[width=3.5in]{all_acc.png}
  \caption{各分类预测模型测试集Accuracy比较}
  \label{fig:side:b}
  \end{minipage}
\end{figure}
从Accuracy视角看，Naive Bayes展现出了最好的效果，这也体现出Naive Bayes在文本分类上的优势。但是在第3个类别上，Naive Bayes做出了较差的预测，它在第三个类别上的Recall和F1-score都达到了所有分类模型的最低值，这也一定程度上反映出Naive Bayes简单模型的劣势，对于某些特殊的数据会表现出较低的敏感性，从而进行错误的预测。总体上来说，四种分类模型在20个类别的预测结果上大致表现出了较相似的分布，在第3个类别上，RF和NB都获得了较高的Precision和较低的Recall和F1-score，而SVM，KNN，NB，RF都在第13个类别上表现不佳，因此这并不是单个分类模型的问题，可能与特征提取方式有关，我们的特征提取方式对于类别3和13的特征并没有很好地提取出来，导致分类模型在这两个类别上表现较差。总体来说，NB表现出了它在文本分类任务上的优势，而RF表现出了它在各个类别上预测结果的稳定性，虽然受限于模型限制的最大深度，但仍然可以获得都还不错的结果，如果可以将决策树深度继续调高，那么结果可以预见地会获得很大的提升。
...
【8】 \href{https://blog.csdn.net/iteye_3893/article/details/82636364}{hadoop2 集群出现过的问题}\par
【9】 \href{https://blog.csdn.net/u011239443/article/details/51728659}{TF-IDF和Word2Vec提取文本特征值}\par
【10】 \href{https://blog.csdn.net/u010839779/article/details/79895698}{
	Hadoop日志详解
}\par
【11】 \href{https://blog.csdn.net/qq_37269094/article/details/83214140}{
	KNN算法分析
}\par
【12】 \href{https://www.jianshu.com/p/4743a4b5b610}{
	SVM多分类之一对多
}\par
\appendix
 ...
 \begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{a1.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{a2.jpg}
	\caption{运行时刻截图2 \label{fig:feature-out}}
\end{figure} 

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{a3.jpg}
	\caption{运行时刻截图3 \label{fig:feature-out}}
\end{figure} 

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{a4.jpg}
	\caption{运行时刻截图4 \label{fig:feature-out}}
\end{figure} 

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{a5.jpg}
