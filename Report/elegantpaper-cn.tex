...
\begin{algorithm}[htb]
  ...
  \begin{algorithmic}[1]
    ...
    \Ensure
    输出训练结果文件，记录键值对<类名\#单词，数量>
  \State Map<String, int> TrainSet
  \State Map<String, int> ClassSet
  \State Set<String> WordSum    //记录词库
  \State Map阶段：分割每个单词并根据文件名获得类名编号，输出<类名\#单词，数量>
  \Function {setup}{$A$, $B$}
    \For{(key, value) in A}
  ...
  \Function {map}{$offset$, $text$}
    \For{each 单词 $w$ in $text$}
      ...
      \For{each 类别 $c$ in ClassSet.keys}
        ...
        \State sumClass = TrainSet.get(currentClass);//类对应的单词总数
        \State sumWord = WordSum.size(); //词库中词数量
        \State result = log(wordcount / (sumClass + sumWord));
        \State Emit(filename, currentClass + "\#" + result);
   		\EndFor
      \State Emit($classnum + "\#" + w$, 1)
   \EndFor
  \EndFunction
...
\begin{algorithm}[htb]
  ...
  \Function {reduce}{$key$, $value$}
    ...
    \For{value in values}
        \State 类名编号classno = value.GetClassNum
